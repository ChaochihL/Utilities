...
\n\
Usage: ./bam_vs_fastq_check [SAM/BAM_file_list] [accession_names_list] [fastq_file_list] [out_dir] [script_dir] \n\
\n\
...
function compare_seq_id() {
    ...
    samtools view "${aligned}" | awk '{print $1}' > "${out_dir}"/"${accession}"_seqIDs.txt
